...
PLI_INT32 alu_sync_callback (p_cb_data cb_data)
{
	...
	if (p_userdata->flag == 0) // if this is the 1st callback occured for this time step
	{	
		// we don't need schedule cbReadWriteSynch for this time step anymore
		p_userdata->flag = 1;
...
PLI_INT32 alu_calltf (PLI_BYTE8 *user_data)
{
	...
	cb_data.obj = a_handle;
	cb_handle = vpi_register_cb(&cb_data);
	vpi_free_object(cb_handle);

	cb_data.obj = b_handle;
	cb_handle = vpi_register_cb(&cb_data);
	vpi_free_object(cb_handle);
	
	cb_data.obj = type_handle;
	cb_handle = vpi_register_cb(&cb_data);
	vpi_free_object(cb_handle);
